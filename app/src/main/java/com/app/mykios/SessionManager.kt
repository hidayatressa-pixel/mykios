...
class SessionManager(context: Context) {
    ...
    fun saveTokoInfo(namaToko: String, namaPemilik: String, kategori: String, password: String) {
        ...
        // Entitlement is controlled by the build edition; never infer PRO from profile data.
    ...
    fun isProfileComplete(): Boolean {
        return !getEmail().isNullOrEmpty() && getAlamatToko() != "Alamat belum diatur"
    }

    fun isPro(): Boolean = BuildConfig.MYKIOS_PRO_EDITION
    ...
    fun setLanguage(lang: String) {
        ...
        editor.putBoolean("is_lang_set", true)
        editor.apply()
    }
    ...
    fun getLanguage(): String {
        return prefs.getString("lang", "in") ?: "in"
    }
    ...
    fun setQrisPath(path: String) {
        prefs.edit().putString("qris_path", path).apply()
    }
    ...
    fun addSaldoDigital(amount: Int) {
        val current = getSaldoDigital()
        prefs.edit().putInt("saldo_digital", current + amount).apply()
    }
    ...
    fun verifyPin(pin: String): Boolean {
        ...
        // One-time migration for users upgrading from versions that stored the PIN as plain text.
    ...
    fun addXP(amount: Int) {
        val current = getXP()
        prefs.edit().putInt("user_xp", current + amount).apply()
    }
